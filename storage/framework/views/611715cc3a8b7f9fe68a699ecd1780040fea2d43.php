					...
					<div class="row mb-4">
						<div class="col-lg-2 col-md-6 col-sm-12 text-left justify-content-left">
							<div class="d-flex " id="template-buttons-group">	
								<a id="save-button-template" class="template-button mr-2" onclick="return saveText(this);" href="#"><i class="fa-solid fa-floppy-disk-pen table-action-buttons table-action-buttons-big delete-action-button" data-tippy-content="<?php echo e(__('Save Document')); ?>"></i></a>				
						...
						<?php if(App\Services\HelperService::checkIntegrationAccess()): ?>
							<?php if(App\Services\HelperService::extensionWordpressIntegration()): ?>
								<?php if(App\Services\HelperService::checkWordpressIntegrationFeature()): ?>
									<div class="col-lg-2 col-md-6 col-sm-12">
										<div class="btn-group mt-1 publish-action-buttons">
											<button type="button" class="btn btn-primary ripple dropdown-toggle" data-bs-toggle="dropdown" id="export" data-bs-display="static" aria-expanded="false" style="text-transform: none; font-size: 11px"><?php echo e(__('Publish')); ?></button>
											<div class="dropdown-menu" aria-labelledby="export" data-popper-placement="bottom-start">								
												<a class="dropdown-item" id="publish" href="#"><i class="fa-brands fa-wordpress fs-13 text-muted mr-2"></i> <?php echo e(__('Publish to Wordpress')); ?></a>
											</div>
										</div>
									</div>
								<?php endif; ?>
...
<script type="text/javascript">
	...
	$(function () {
		...
		$('#publish').on('click', function(e) {
			...
			if (!saveButton.hasAttribute('target')) {
				toastr.warning('<?php echo e(__("Make sure to save your generated content first")); ?>');
				return;
			} else {
				...
				<?php if(App\Services\HelperService::checkIntegrationAccess()): ?>
					<?php if(App\Services\HelperService::extensionWordpressIntegration()): ?>
						<?php if(App\Services\HelperService::checkWordpressIntegrationFeature()): ?>
						let url = "<?php echo e(route('user.integration.wordpress.post.create', ['id' => ':id'])); ?>";
						<?php endif; ?>
					<?php endif; ?>
				<?php endif; ?>
	...
	function saveText(event) {
		...
		if (!event.target) {
			toastr.warning('<?php echo e(__('You will need to create an article first before saving your changes')); ?>');
			document.querySelector('#loader-line')?.classList?.add('hidden');  
		} else {
			$.ajax({
				headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/customer/www/staging.paraclete.ai/public_html/resources/views/default/user/templates/original-template.blade.php ENDPATH**/ ?>