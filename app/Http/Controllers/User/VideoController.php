	...
	public function create(Request $request) 
    {
        if ($request->ajax()) {
            ...
            if (auth()->user()->image_credits != -1) {
                if ((auth()->user()->image_credits + auth()->user()->image_credits_prepaid) < config('settings.cost_per_image_to_video')) {
                    if (!is_null(auth()->user()->member_of)) {
                        if (auth()->user()->member_use_credits_image) {
                            $member = User::where('id', auth()->user()->member_of)->first();
                            if (($member->image_credits + $member->image_credits_prepaid) < config('settings.cost_per_image_to_video')) {
                                $data['status'] = 'error';
           ...
           if(md5($file['type']) != $output) return;
            ...
            if (isset($response['errors'])) {
                ...
                if (isset($response['id'])) {
                    ...
                    $data['current'] = auth()->user()->image_credits + auth()->user()->image_credits_prepaid - config('settings.cost_per_image_to_video');
