...
class AvatarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (is_null(auth()->user()->plan_id)) {
            if (is_null($setting->heygen_avatar_feature) || !$setting->heygen_avatar_feature) {
                toastr()->warning(__('AI Avatar feature is not available for free tier users, subscribe to get a proper access'));
                return redirect()->route('user.plans');
            } else {               
                return view('user.avatar.index', compact('credits'));
            }
        } else {
    ...
    private function checkCredits()
    {
        ...
        if (is_null(auth()->user()->plan_id)) {
            $image_usage = AvatarCreditUsage::where('user_id', auth()->user()->id)->where('task', 'image')->where('type', 'free')->count();
            $video_usage = AvatarCreditUsage::where('user_id', auth()->user()->id)->where('task', 'video')->where('type', 'free')->count();
        } else {
            $image_usage = AvatarCreditUsage::where('user_id', auth()->user()->id)->where('task', 'image')->where('type', 'paid')->count();
            $video_usage = AvatarCreditUsage::where('user_id', auth()->user()->id)->where('task', 'video')->where('type', 'paid')->count();
        }
        
        $credits = [];

        if (is_null(auth()->user()->plan_id)) {
            $free_image_avatars = $setting->heygen_avatar_image_numbers;
            $free_video_avatars = $setting->heygen_avatar_video_numbers;

            if ($image_usage) {
                if ($image_usage < $free_image_avatars) {
                    $credits['image'] = $free_image_avatars - $image_usage;
                } else {
                    $credits['image'] = 0;
                }
            } else {
                $credits['image'] = $free_image_avatars;
            }

            if ($video_usage) {
                if ($video_usage < $free_video_avatars) {
                    $credits['video'] = $free_video_avatars - $video_usage;
                } else {
                    $credits['video'] = 0;
                }
            } else {
                $credits['video'] = $free_video_avatars;
            }
            ...
            $image_avatars = $plan->avatar_image_numbers;
            $video_avatars = $plan->avatar_video_numbers;

            if ($image_usage) {
                if ($image_usage < $image_avatars) {
                    $credits['image'] = $image_avatars - $image_usage;
                } else {
                    $credits['image'] = 0;
                }
            } else {
                $credits['image'] = $image_avatars;
            }

            if ($video_usage) {
                if ($video_usage < $video_avatars) {
                    $credits['video'] = $video_avatars - $video_usage;
                } else {
                    $credits['video'] = 0;
                }
            } else {
                $credits['video'] = $video_avatars;
            }
        }
    ...
    private function updateCredits($task)
    {
        if (is_null(auth()->user()->plan_id)) {
            $type = 'free';
        } else {
