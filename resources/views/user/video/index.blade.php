			...
			<div class="col-lg-4 col-md-12 col-sm-12">
				<div class="card border-0">
					<div class="card-header pt-4 border-0" id="video-image-counter">
						...
						<p class="fs-11 text-muted mb-0 text-right"><i class="fa-sharp fa-solid fa-bolt-lightning mr-2 text-primary"></i>{{ __('Your Balance is') }} <span class="font-weight-semibold" id="balance-number">@if (auth()->user()->image_credits == -1) {{ __('Unlimited') }} @else {{ number_format(auth()->user()->image_credits + auth()->user()->image_credits_prepaid) }} {{ __('credits') }}@endif</span></p>
					</div>
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('#resultTable').DataTable({
				...
				colReorder: true,
				language: {
					"emptyTable": "<div><img id='no-results-img' src='{{ theme_url('img/files/no-result.png') }}'><br>{{ __('No image to video results created yet') }}</div>",
