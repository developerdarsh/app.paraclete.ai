					...
					<div class="row">
						<div class="col-sm-12">
							<div class="avatar-banner">
								<h4 class="page-title mb-0 mt-2"><i class="text-primary mr-2 fs-16 fa-solid fa-aperture"></i> {{ __('AI Avatar') }}</h4>
								<h6 class="text-muted">{{ __('Produce studio-quality videos in 175 languages without a camera or crew') }}</h6>
								...
								<div class="avatar-credits">
									<span class="text-muted fs-12">{{__('Credits')}}: </span> <span class="fs-12 text-primary">{{$credits['image']}}</span> <span class="text-muted fs-12">{{__('Avatars from Image')}}</span> <span class="text-muted fs-12">|</span> <span class="fs-12 text-primary">{{$credits['video']}}</span> <span class="text-muted fs-12">{{__('Avatars from Video')}}</span>
								</div>
							</div>
						</div>
						<div class="col-lg-2 col-md-3 col-sm-12">							
							<a id="save-button" class="avatar-action-btn text-center mt-6" href="{{ route('user.extension.avatar') }}"><i class="fa-solid fa-clapperboard-play mr-4"></i>{{ __('Create Video') }}</a>
