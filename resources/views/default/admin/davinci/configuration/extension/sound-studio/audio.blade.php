...
<div class="page-header mt-5-7 justify-content-center">
	<div class="page-leftheader text-center">
		<h4 class="page-title mb-0">{{ __('Sound Studio Background Audio') }}</h4>
...
@section('content')	
	<div class="row justify-content-center">
		...
		<div class="col-lg-8 col-md-12 col-xm-12 mt-4">
			<div class="card">
...
@section('js')
	...
	<script src="{{theme_url('js/sound-studio.js')}}"></script>
	<!-- Awselect JS -->
	...
	<script type="text/javascript">
		$(function () {
			...
			let list = $('#backgroundMusicTable').DataTable({
				...
				colReorder: true,
				language: {
					"emptyTable": "<div><img id='no-results-img' src='{{ theme_url('img/files/no-result.png') }}'><br>{{ __('There are no music files uploaded yet') }}</div>",
				...
				processing: true,
				serverSide: true,
				ajax: "{{ route('admin.davinci.configs.sound.audio.list') }}",
				columns: [{
						data: 'created-on',
						name: 'created-on',
						orderable: true,
			...
			$(document).on('click', '.makePrivateButton', function(e) {
				...
				$.ajax({
					...
					method: 'post',
					url: '/app/admin/davinci/configs/sound-studio/audio/private',
					data: formData,
					processData: false,
					contentType: false,
			...
			$(document).on('click', '.makePublicButton', function(e) {
				...
				$.ajax({
					headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
					method: 'post',
					url: '/app/admin/davinci/configs/sound-studio/audio/public',
					data: formData,
					processData: false,
					contentType: false,
					success: function (data) {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
							method: 'post',
							url: '/app/admin/davinci/configs/sound-studio/audio/delete',
							data: formData,
							processData: false,
							contentType: false,
							success: function (data) {
