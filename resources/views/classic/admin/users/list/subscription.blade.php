					...
					<form method="POST" action="{{ route('admin.user.assign', [$user->id]) }}" enctype="multipart/form-data">
						...
						<div class="row">
							...
							<div class="col-sm-12 col-md-12">
								<div>
									<p class="fs-13 mb-2">{{ __('User Full Name') }}: <span class="font-weight-bold ml-1">{{ $user->name }}</span></p>
									<p class="fs-13 mb-2">{{ __('User Email Address') }}: <span class="font-weight-bold ml-1">{{ $user->email }}</span></p>
									<p class="fs-13 mb-2">{{ __('Registered On') }}: <span class="font-weight-bold ml-1">{{ $user->created_at }}</span></p>
									<p class="fs-13 mb-4">{{ __('Current subscription plan of the user') }}: <span class="font-weight-bold ml-1">{{ $plan }}</span></p>
								</div>

								<div class="input-box">
									<label class="form-label fs-13">{{ __('Subscription Plans') }}</label>
