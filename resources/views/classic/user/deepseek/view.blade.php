...
@section('page-header')
	...
	<div class="page-header mt-5-7">
		<div class="page-leftheader">
			<h4 class="page-title mb-0">Rabbit Hole Chat Researcher</h4>
			<ol class="breadcrumb mb-2">
				<li class="breadcrumb-item"><a href="{{route('user.dashboard')}}"><i class="fa-solid fa-messages-question mr-2 fs-12"></i>{{ __('User') }}</a></li>
				<li class="breadcrumb-item" aria-current="page"><a href="{{route('user.deepseek')}}"> {{ __('Deepseek AI') }}</a></li>
				<li class="breadcrumb-item active" aria-current="page"><a href="{{url('#')}}">Rabbit Hole Chat Researcher</a></li>
			</ol>
		</div>
							...
							<div class="col-md-12 col-sm-12" >									
								<div id="chat-container">
									<div class="msg left-msg">
										<div class="message-img" style=""></div>
									 ...
									 <div class="chat-box" id="chat-box">
                                        <div class="message bot-message"> Hello! How can I help you?</div>
                                            @foreach($chatHistories as $chat)
                                                <div class="message user-message"><strong></strong> {{ $chat->message }}</div>
                                                <div class="message bot-message"><strong></strong> {{ $chat->response }}</div>
                                            @endforeach
                                        </div>
                                    </div>
									...
									<div class="chat-controllers">	
										...
										<div class="text-input">					    
                     					...
                     					<div class="action-0btn">
											...
											<div>
												<button class="btn ripple chat-button" id="chat-button">{{ __('Send') }} <i class="fa-solid fa-paper-plane-top ml-1"></i></button>
											</div>
										</div> 
                  					</div>
								</div> 
	...
	function process(message,webAccessBtn) {
		...
		fetch('/app/user/chat/process', {
			...
			.then(data => {
				...
				eventSource = new EventSource("/app/user/chat/generate?conversation_id=" + active_id + "&web_access="+webAccessBtn);				
					...
					if (e.data == "[DONE]") {
						...
						if (uploaded_image == '') {
							if (model == 'claude-3-5-haiku-20241022' || model == 'claude-3-5-sonnet-20241022' || model == 'claude-3-opus-20240229' || model == 'gemini_pro' || model == 'o1-mini' || model == 'o1-preview') {
								...
								isProcess = JSON.parse(e.data).choices[0].delta.process;
							}
