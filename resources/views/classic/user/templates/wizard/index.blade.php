...
<form id="wizard-form" action="" method="post" enctype="multipart/form-data" class="mt-24"> 		
	...
	<div class="row justify-content-center">	
		<div class="col-sm-12 text-center">
			<h3 class="page-title">{{ __('AI Article Wizard') }}</h3>
			<div class="mb-6">
				<x-balance-template />
			</div>
		</div>
										...
										<select id="language" name="language" class="form-select" data-placeholder="{{ __('Select input language') }}">		
											@foreach ($languages as $language)
												@if (!is_null($wizard['language']))
													<option value="{{ $language->language_code }}" data-img="{{ theme_url($language->language_flag) }}" @if ($wizard['language'] == $language->language) selected @endif> {{ $language->language }}</option>
												@else
