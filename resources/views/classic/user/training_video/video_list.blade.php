									...
									@if($video->section == 0)
										...
										<div class="video-list">
											...
											<div class="py-3 video-caption">
												...
												<a href="{{ url('/app/user/videos/download') }}?link={{ urlencode($embedUrl) }}&title={{ urlencode($video->title) }} class="font-weight-bold"><i class="fas fa-download"></i></a>
											</div>
					...
					<table id='listVideos' class='table listVideos' width='100%'>
						...
						<tbody>
							@foreach($videos as $video)
								...
								<tr>
									<td>{{ $video->title }}</td>
									<td><a href="{{ url('/app/user/videos/pdfdownload') }}?link={{ urlencode($video->file_link) }}&title={{ urlencode($video->title) }}&type={{ $video->files }}" download="{{ $video->title }}">Download</a></td>
								</tr>
								@endif
							@endforeach
						</tbody>
